...
# --- 1. 配置参数 ---
DATA_FILES_PATH = "/home/users/nus/e1352689/scratch/stack_edu_subset/"
TOKENIZER_PATH = "/home/users/nus/e1352689/pretrain-LLM-from-scratch/tokenizer"
PROCESSED_DATA_PATH = "/home/users/nus/e1352689/scratch/stack_edu_processed"
BLOCK_SIZE = 3072
...
def prepare_and_tokenize_dataset():
    ...
    for folder in languages:
        dataset_dir = os.path.join(DATA_FILES_PATH, folder)
        datasets.append(load_from_disk(dataset_dir))
    # 仅提示一个示例（如果有）
    raw_ds = concatenate_datasets(datasets)
    try:
        ex = next(iter(raw_ds))
        print(f"示例行：content[:80]={ex.get('content','')[:80]!r}")
    ...
    filtered_ds = raw_ds.select_columns(["content"])

    print("\n=== 步骤 2/3: 加载 Tokenizer 并进行分词 ===")
