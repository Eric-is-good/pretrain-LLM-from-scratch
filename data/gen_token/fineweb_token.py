...
# --- 1. 配置参数 ---
DATA_FILES_PATH = "/home/users/nus/e1352533/mydata/data/fineweb/"
TOKENIZER_PATH = "/home/users/nus/e1352533/code/pretrain-LLM-from-scratch/tokenizer"
PROCESSED_DATA_PATH = "/home/users/nus/e1352533/mydata/data/fineweb/processed_data_2"
BLOCK_SIZE = 3072
...
def prepare_and_tokenize_dataset():
    ...
    print("\n=== 步骤 3/5: 按 score>0.5 筛选样本 ===")
    before = len(raw_ds)
    filtered_ds = raw_ds.filter(
        lambda x: x["score"] is not None and x["score"] > 0.5 and isinstance(x["content"], str) and len(x["content"]) > 0,
        num_proc=NUM_PROC,
        desc="Filtering by score>0.5",
    )
