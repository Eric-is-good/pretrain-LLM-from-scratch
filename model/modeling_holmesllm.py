...
from .configuration_holmesllm import HolmesLLMConfig
...
_CONFIG_FOR_DOC = "HolmesLLMConfig"
...
class HolmesAttention(nn.Module):
    ...
    def __init__(self, config: HolmesLLMConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class HolmesSdpaAttention(HolmesAttention):
    """
    Holmes attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `HolmesAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from HolmesAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "HolmesModel is using HolmesSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class HolmesLLMModel(HolmesPreTrainedModel):
    ...
    def __init__(self, config: HolmesLLMConfig):
        ...
        self.layers = nn.ModuleList(
            [HolmesDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class HolmesLLMForCausalLM(HolmesPreTrainedModel):
    ...
    def forward(
        ...
        >>> model = HolmesForCausalLM.from_pretrained("meta-holmes/Holmes-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-holmes/Holmes-2-7b-hf")
...
class HolmesLLMForQuestionAnswering(HolmesPreTrainedModel):
    ...
    # Copied from transformers.models.bloom.modeling_bloom.BloomForQuestionAnswering.__init__ with Bloom->Holmes
    def __init__(self, config):
        super().__init__(config)
        self.transformer = HolmesLLMModel(config)
